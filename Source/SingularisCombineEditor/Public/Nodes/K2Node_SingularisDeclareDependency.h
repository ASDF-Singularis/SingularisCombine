 ...
 * ExpandNode 两步展开：策略 GetDependencyProvider → 组件 GetDeclaredComponent（查询能力下沉于组件，
 * 策略不承载查询逻辑）；编译期 hook 扫描本节点按 (Scope, Class) 去重写入 CDO。
 */
UCLASS()
...
public:
	...
	/**
	 * 编译展开：替换为「策略获取依赖提供者 → 组件查询声明组件」两步中间调用链并迁移链接
